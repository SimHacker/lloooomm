...
**Navigation**: [Play-Learn-Lift Strategy](#play-learn-lift-strategy) • [Sister Script Pattern](#sister-script-pattern) • [LLOOOOMM IMPORT](#lloooomm-import) • [Empathic SQL](#constitutional-sql) • [Iterative Refinement](#iterative-refinement)
...
PROJECT_IDS = ["leela-devops-0", "toledo-staging", "manhattan-dev"]
...
PROJECT_IDS = ["leela-devops-0", "toledo-staging", ...]
...
### **⚡ Quick Start PARA Implementation**

**Week 1: Audit Current Documents**
1. List all active LLOOOOMM documents
2. Categorize into Projects/Areas/Resources/Archives
3. Apply 10-15 rule to Projects category

**Week 2: Implement Weekly Review**
1. Schedule 30-minute weekly LLOOOOMM review
2. Follow 5-step process consistently
3. Track which documents make progress

**Week 3: Optimize for Actionability**
1. Reorganize documents by actionability, not topic
2. Add clear completion criteria to all projects
3. Move stalled projects to Resources or Archives

**Week 4: Build Network Effects**
1. Add LLOOOOMM IMPORT directives between related documents
2. Create shared modules for common patterns
3. Document cross-pollination opportunities

### **🎯 Success Metrics**

**Weekly:** Complete 1+ projects, progress on 80% of active documents, 30-min reviews, max 15 projects
**Monthly:** Increased automation, regular completions, cross-document insights  
**Quarterly:** Measurable business impact, methodology improvements, team adoption

---

*For more on the PARA Method, see Tiago Forte's work at [fortelabs.com](https://fortelabs.com) and his books "Building a Second Brain" and "The PARA Method".*

---

## 🏗️ **Bootstrapping LLOOOOMM into PARA Structure**

**Practical guide to organizing and modularizing LLOOOOMM documents**

### **📋 Starting Point: Single Document Assessment**

**Before restructuring, evaluate your current LLOOOOMM document:**

```markdown
# Current State Analysis
## Document: cloud-optimization.md (example)

**Content Inventory:**
- [ ] Procedures (manual steps)
- [ ] Data tables (project lists, costs)
- [ ] Code blocks (bash commands, scripts)
- [ ] Reference materials (links, documentation)
- [ ] Decision logs (why choices were made)
- [ ] Results (actual outcomes, metrics)
...
**Identify what belongs where in PARA structure:**

#### **Projects (Active Work)**
```bash
# Extract active procedures into project documents
grep -A 10 "## Current.*" cloud-optimization.md > projects/q1-cost-reduction.md
grep -A 5 "### Next Steps" cloud-optimization.md >> projects/q1-cost-reduction.md
```

#### **Resources (Reusable Components)**
```bash
# Extract reusable patterns
grep -A 20 "## Standard Procedure" cloud-optimization.md > resources/modules/cost-analysis-template.md
grep -A 15 "### Common Commands" cloud-optimization.md > resources/patterns/gcp-commands.md
```

#### **Archives (Completed Work)**
```bash
# Move completed iterations
grep -A 30 "## December Analysis" cloud-optimization.md > archives/2024-q4/december-optimization.md
```

### **📁 Phase 2: Create PARA Directory Structure**

**Set up organized file system:**

```bash
# Create PARA structure for LLOOOOMM
mkdir -p lloooomm/{projects,areas,resources,archives}
mkdir -p lloooomm/resources/{modules,patterns,templates,reference}
mkdir -p lloooomm/archives/{2024-projects,lessons-learned,performance-data}

# Initialize with README files
echo "# Active LLOOOOMM Projects (10-15 max)" > lloooomm/projects/README.md
echo "# Ongoing LLOOOOMM Responsibilities" > lloooomm/areas/README.md
echo "# Reusable LLOOOOMM Components" > lloooomm/resources/README.md
echo "# Completed LLOOOOMM Wisdom" > lloooomm/archives/README.md
```
...
| Project | Cost | Status | Owner |
|---------|------|--------|-------|
| prod-web | $1,234 | active | alice |
| staging-api | $567 | review | bob |

<!-- Becomes modular data file -->
# resources/data/project-costs.md
# LLOOOOMM Data: Project Cost Tracking
<!-- This data is imported by multiple documents -->

| Project | Cost | Status | Owner | Last Updated |
|---------|------|--------|-------|--------------|
| prod-web | $1,234 | active | alice | 2024-01-15 |
| staging-api | $567 | review | bob | 2024-01-14 |
```

#### **Extract Command Patterns**
```markdown
# resources/patterns/gcp-analysis.md
# LLOOOOMM Pattern: GCP Resource Analysis

## Standard Commands
```bash
# Project discovery
gcloud projects list --quiet --format=json | jq -r '.[].projectId' | cat

# Cost analysis  
gcloud billing accounts list --quiet --format=csv | cat

# Resource inventory
gcloud compute instances list --quiet --format=csv | cat
```

## Usage
```markdown
# LLOOOOMM IMPORT ../resources/patterns/gcp-analysis.md#standard-commands
```
```

#### **Extract Procedures**
```markdown
# resources/modules/cost-optimization-procedure.md
# LLOOOOMM Module: Cost Optimization Procedure

## Phase 1: Discovery
1. **Inventory Projects**
   # LLOOOOMM IMPORT ../patterns/gcp-analysis.md#project-discovery
   
2. **Analyze Costs**
   # LLOOOOMM IMPORT ../patterns/gcp-analysis.md#cost-analysis
...
2. **Calculate Savings**
   # LLOOOOMM IMPORT ../data/project-costs.md#cost-table [calculate-savings]
...
### **🔗 Phase 4: Establish Import Relationships**

**Connect modular components with LLOOOOMM IMPORT directives:**

#### **Project Document Structure**
```markdown
# projects/q1-cost-reduction.md
# Q1 2024 Cost Reduction Project

## Objective
Reduce cloud costs by 25% while maintaining performance

## Methodology
# LLOOOOMM IMPORT ../resources/modules/cost-optimization-procedure.md

## Current Data
# LLOOOOMM IMPORT ../resources/data/project-costs.md#cost-table

## Progress Tracking
- [x] Phase 1: Discovery (completed 2024-01-10)
...
```python
# projects/q1-cost-reduction.py
# Sister script for Q1 Cost Reduction project

import json
from pathlib import Path

def load_lloooomm_data(file_path, section=None):
    """Load data from LLOOOOMM IMPORT directive"""
    # Implementation for parsing markdown tables
    pass

def update_project_status():
    """Update project document with current status"""
    # Read current data
    costs = load_lloooomm_data("../resources/data/project-costs.md", "cost-table")
    
    # Perform analysis
    total_cost = sum(float(row['Cost'].replace('$', '')) for row in costs)
    
    # Update project document
    status_update = f"""
## Latest Analysis (Updated: {datetime.now().strftime('%Y-%m-%d')})

**Total Monthly Cost:** ${total_cost:,.2f}
**Optimization Target:** ${total_cost * 0.25:,.2f} (25% reduction)
**Progress:** {calculate_progress()}%
"""
    
    # Append to project document
    with open("q1-cost-reduction.md", "a") as f:
        f.write(status_update)
```

### **📊 Phase 5: Organize Supporting Files**

**Structure non-markdown assets:**

```bash
# Create supporting file structure
mkdir -p lloooomm/resources/data/{raw,processed,exports}
mkdir -p lloooomm/resources/scripts/{analysis,automation,utilities}
mkdir -p lloooomm/resources/configs/{templates,environments,settings}

# Organize existing files
mv *.json lloooomm/resources/data/raw/
mv *.csv lloooomm/resources/data/processed/
mv *.py lloooomm/resources/scripts/analysis/
mv *.yaml lloooomm/resources/configs/templates/
```

### **🔄 Phase 6: Establish Maintenance Workflow**

**Create sustainable update processes:**

#### **Weekly Review Checklist**
```markdown
# areas/lloooomm-maintenance.md
# LLOOOOMM PARA Maintenance

## Weekly Review Process
- [ ] **Projects**: Review active documents (max 15)
- [ ] **Resources**: Update shared data files
- [ ] **Modules**: Refine reusable procedures
- [ ] **Archives**: Move completed work

## Monthly Cleanup
- [ ] **Consolidate**: Merge similar resources
- [ ] **Prune**: Remove outdated references
- [ ] **Optimize**: Improve IMPORT directives
- [ ] **Document**: Update lessons learned

## Quarterly Restructure
- [ ] **Assess**: Evaluate PARA organization effectiveness
- [ ] **Reorganize**: Adjust structure based on usage
- [ ] **Archive**: Move old projects to archives
- [ ] **Plan**: Identify new resource needs
```

### **🎯 Phase 7: Validation and Testing**

**Ensure modular structure works:**
...
def validate_imports(file_path):
    ...
    imports = re.findall(r'# LLOOOOMM IMPORT ([^\\n]+)', content)
    
    for import_line in imports:
        source_file = import_line.split('#')[0].strip()
...
for project in Path('lloooomm/projects').glob('*.md'):
    print(f'\\n📋 Validating: {project.name}')
    validate_imports(project)
"
```

### **📈 Success Metrics**

**Before:** Single 847-line document, repeated procedures, mixed concerns  
**After:** 12 focused documents (avg 70 lines), reusable modules, clear separation  
**Improvements:** 3.2x reuse factor, 40% less maintenance time, 60% faster onboarding, 50% fewer errors

---

*Modularizing LLOOOOMM documents into PARA structure transforms monolithic documents into maintainable, reusable knowledge systems while preserving the document-as-program methodology.*
...
**LLOOOOMM documents become more useful when connected**:

#### **Template Libraries**
- **Reusable patterns** for common tasks
- **Inheritance hierarchies** - Specialized documents extending base templates
- **Cross-domain insights** - Learnings from one area improving others

#### **Community Benefits**
- **Shared wisdom** - Collective learning from many practitioners
- **Pattern evolution** - Best practices emerge through usage
- **Tool ecosystem** - Scripts and utilities for common needs

### **📊 Success Metrics**

**Ways to measure LLOOOOMM effectiveness**:

#### **Quantitative Measures**
- **💰 Cost Impact**: Direct financial benefits from optimizations
- **⚡ Time Savings**: Manual → automated workflow improvements  
- **🎯 Error Reduction**: Fewer mistakes through systematization
- **📈 Knowledge Velocity**: Speed of learning and insight generation

#### **Qualitative Indicators**
- **🧠 Understanding**: How well do you understand your systems?
- **🔄 Adaptability**: How quickly can you respond to changes?
- **🤝 Collaboration**: How well do teams work together?
- **💡 Discovery Rate**: How often do you find new opportunities?

---

## 🎯 **LLooMM Syntax Protocol**

### **The DDOOUUBBLLEE Convention**

**Core Principle**: Keywords and language names CAN be doubled for emphasis and disambiguation, like EXTRA STRONG PARENTHESES. The system understands regular, doubled, pigeon, and fuzzy variations.

### **Syntactic Flexibility**
- **Regular**: `if`, `for`, `define` - Normal keywords work fine
- **DDOOUUBBLLEE**: `IIFF`, `FFOORR`, `DDEEFFIINNEE` - Unambiguous emphasis
- **Natural**: "when", "for each", "create" - Fuzzy understanding
- **Emoji**: ❓, 🔁, 📝 - Visual programming
- **Pigeon**: "if no good then delete" - Pidgin dialects understood

### **Language Doubling**
- **JJSS**: Double JavaScript - "goes to eleven", understands sloppy syntax
- **TTYY**: Double TypeScript - Meta-types, fuzzy type inference  
- **PPSS**: Double PostScript - Stack operations with natural language
- **CC++++**: Double C++ - Extra meta programming power
- **PPyytthhoonn**: Double Python - Indentation optional, intent understood

### **Pretty Printing**
The LLM can transform messy "doublescript" into canonical, well-formatted code:
- Fixes pigeon syntax → proper grammar
- Normalizes doubled keywords → standard or DDOOUUBBLLEE as appropriate
- Adds helpful comments and structure
- Preserves intent while improving clarity

### **Visual Hierarchy**
1. **DDOOUUBBLLEE** - Maximum clarity, production code
2. **Regular** - Standard writing
3. **Mixed** - Natural emphasis: "The main LLOOOOPP runs until done"
4. **Fuzzy** - Whatever feels natural

### **Parser Protocol**
```yaml
recognition: "fuzzy_matching"
confidence_threshold: 0.7
disambiguation: "context_aware"
pretty_print: "on_request"
```

### **Code Block Language Conventions**

**Use Standard Language Names for Syntax Support:**
- Use `python`, `javascript`, `sql`, `yaml`, `json` (not doubled versions)
- Enables proper syntax highlighting and editor support
- Maintains compatibility with standard markdown processors

**LLM Execution Philosophy:**
When processing code blocks, the LLM should be:
- **Forgiving about syntax** - Infer user intent from imperfect code
- **Strict about active comments** - Treat comments as ultimate truth and instructions
- **Contextually intelligent** - Derive parameters and references from surrounding context
- **Convenience-oriented** - Implement fuzzy table/parameter references automatically

**Active Comments as Truth:**
```python
# This comment describes the real intent - implement this exactly
# Even if the code below has syntax errors or is incomplete
def analyze_costs():
    # LLM: Extract all projects with monthly_cost > threshold
    # LLM: Group by environment (prod, staging, dev)
    # LLM: Calculate savings potential for each group
    projects = get_projects()  # LLM will implement this properly
```

**Fuzzy Reference Resolution:**
- **Table names**: "projects" → finds project_data, projects_table, etc.
- **Parameter names**: "threshold" → finds cost_threshold, min_cost, etc.
- **Context derivation**: Infers missing parameters from document context
- **Smart defaults**: Uses reasonable defaults when parameters unclear

**Interactive Teaching Philosophy:**
The LLM acts as a patient, enthusiastic programming mentor:
- **Kind and constructive** - Never critical, always supportive and encouraging
- **Gentle guidance** - Teaches by example and positive reinforcement
- **Interactive discussion** - Asks questions to understand intent and teach concepts
- **Enthusiastic suggestions** - Shares exciting language features and techniques
- **Concept introduction** - Introduces new programming concepts naturally
- **Best practice modeling** - Shows good practices through improved code examples

**Discussion Flow:**
1. **Analyze code** - Identify areas for improvement and learning opportunities
2. **Ask clarifying questions** - "What did you want this function to accomplish?"
3. **Suggest improvements** - "Here's a more elegant way to do this..."
4. **Teach concepts** - "This is a great place to learn about list comprehensions!"
5. **Show examples** - Demonstrate improved code with explanations
6. **Encourage experimentation** - "Try this pattern and see how it feels!"

**Code Improvement Areas:**
- **Style and formatting** - Consistent indentation, naming conventions
- **Clarity and readability** - Better variable names, clear logic flow
- **Language idioms** - Pythonic patterns, JavaScript best practices
- **Performance optimization** - Efficient algorithms, memory usage
- **Error handling** - Robust exception handling, input validation
- **Documentation** - Clear comments, docstrings, type hints
- **Modern features** - Latest language features and libraries

### **🏗️ Object Editing Protocol Tokens** {#object-editing-protocol}

**LLOOOOMM uses special ID and COMMAND tokens as magic processing instructions for object lifecycle management. These tokens act as forcing functions and state declarations that prevent errors and make intentions explicit.**

> **⚠️ SAFETY WARNING FOR MAINTAINERS**: This documentation deliberately avoids executable examples of meta-processing keywords to prevent accidental execution. When documenting command syntax, always describe in words and use "double quotes" around tokens like "COMMAND", "BEGIN", "END", etc. Never show raw executable examples that an LLM might accidentally interpret as instructions to execute. This separation between documentation and execution is critical for safety.

#### **Universal Primary ID Conventions**

**These conventions work with any field inferred to be the primary identifier:**

```yaml
# Works with explicit id field
- id: "PROTOTYPE"
  name: "TemplateCharacter"

# Works with name as primary ID  
- name: "PROTOTYPE"
  character_type: "template"

# Works with any primary identifier
- session_id: "DELETE"
...
- command_name: "ARCHIVE"
  preserve_before_removal: true
```

**Primary ID Inference Rules:**
1. **Explicit `id` field** - Always primary if present
2. **`name` field** - Primary if no explicit `id`
3. **Context-specific IDs** - `session_id`, `location_id`, `character_id`, etc.
4. **First unique identifier** - LLM infers from context

#### **Space-Separated Token Syntax**

**Apply commands while preserving original IDs by adding tokens after a space:**

```yaml
# Preserve ID while applying command
- id: "barney_session_2024_01_15 ARCHIVE"
  name: "Barney"
  status: "completed"
  # → Archives object but preserves original ID for reference

- name: "murphy_bartender DELETE"
  role: "bartender"
  reason: "Character being replaced"
  # → Marks for deletion while keeping original name

- session_id: "beer_run_monday PENDING"
  participants: ["barney", "rookie"]
  # → Needs attention but keeps session identifier

- location_name: "old_dive_bar DEPRECATED"
  replacement: "new_sports_bar"
  # → Being phased out but name preserved for historical reference
```

**Benefits of Space-Separated Syntax:**
- **ID Preservation**: Original identifier remains intact for references
- **Command Application**: Token still triggers appropriate processing
- **Historical Continuity**: Archived objects maintain their original IDs
- **Reference Integrity**: Links to objects don't break during lifecycle changes
- **Audit Trail**: Clear history of what happened to which specific object

#### **Core Lifecycle ID Tokens**

**`id: "PROTOTYPE"`** - Template Objects
- **Purpose**: Mark objects as templates/prototypes
- **Forcing Function**: MUST be removed when cloning to create live instances
- **Behavior**: Excluded from live object resolution, used only as templates

**`id: "IGNORE"`** - Ignored Objects
- **Purpose**: Exclude objects from processing without being prototypes
- **Use Cases**: Commented-out objects, disabled features, experimental code
- **Behavior**: Completely ignored by LLM, not considered as viable prototypes

**`id: "PENDING"`** - Temporary Objects
- **Purpose**: Objects that need unique IDs assigned ASAP
- **Forcing Function**: Signals immediate attention required
- **Use Cases**: Rapid prototyping, incomplete object creation
- **Auto-resolution**: Can be resolved with GENERATE_ID command to create appropriate IDs from names

**`id: "DELETE"`** - Deletion Intent
- **Purpose**: Mark objects for deletion
- **Forcing Function**: Clear deletion intent without immediate removal
- **Use Cases**: Cleanup workflows, batch deletion, review before removal

**`id: "ARCHIVE"`** - Archive Then Delete
- **Purpose**: Archive object before deletion (preserve history)
- **Workflow**: Save to archive → Remove from active system
- **Use Cases**: Historical preservation, audit trails, rollback capability

#### **Extended Workflow ID Tokens**

**`id: "DRAFT"`** - Work in Progress
- **Purpose**: Objects under active development

**`id: "REVIEW"`** - Needs Review
- **Purpose**: Objects requiring approval/review

**`id: "BROKEN"`** - Known Issues
- **Purpose**: Objects with known problems

**`id: "DEPRECATED"`** - Phasing Out
- **Purpose**: Objects being phased out gradually

#### **COMMAND System - Declarative Data Transformations**

**The COMMAND system enables declarative data transformations by embedding commands directly in data structures.**

**Basic Pattern:**
Commands are embedded in data structures, processed to transform the data, then the command keys are removed leaving the transformed result.

**Core COMMAND Types:**

**`COMMAND: "DELETE"`** - Same as `id: "DELETE"`
- Transform object to deletion-ready state
- Equivalent to setting primary ID to "DELETE"
- Alternative: `id: "original_id DELETE"` preserves original ID

**`COMMAND: "ARCHIVE"`** - Same as `id: "ARCHIVE"`
- Move to archive, preserve history
- Equivalent to setting primary ID to "ARCHIVE"
- Alternative: `id: "original_id ARCHIVE"` preserves original ID

**Core Command Types:**
- **GENERATE** - Create from template
- **MERGE** - Combine objects  
- **VALIDATE** - Run validation
- **CLONE** - Create instance from prototype
- **ALPHABETIZE** - Sort object keys (also accepts "ABC" or "ALPHABETISE")
- **GENERATE_ID** - Auto-generate ID from name using appropriate style patterns

**ID Generation from Names:**
The GENERATE_ID command creates IDs from names following the style patterns used by other IDs of the same type:

**Pattern Detection:**
- Analyzes existing IDs of the same object type to determine naming conventions
- Detects patterns like snake_case, kebab-case, camelCase, or custom formats
- Identifies common prefixes, suffixes, or structural elements
- Recognizes incrementing patterns and numbering schemes

**Style Examples:**
- **Session IDs**: `beer-run-2024-01-15-19-30-45` → `beer-run-2024-01-16-14-22-33`
- **Character IDs**: `barney_philosopher_001` → `rookie_beginner_002`  
- **Location IDs**: `murphys-bar-main` → `tonys-diner-main`
- **Object IDs**: `item_wallet_player_001` → `item_phone_player_002`

**Collision Handling:**
- Automatically adds numeric suffixes to prevent ID collisions
- Increments existing numbering patterns appropriately
- Maintains consistency with established numbering schemes
- Preserves semantic meaning while ensuring uniqueness

#### **Interactive Command Types**

**QUESTION/ASK** - Ask and answer about data, strips the question key and provides answer with relevant data

**TRY** - Preview operations in chat before applying, shows what would happen and asks for confirmation

**QUERY** - Execute SSQQLL query and replace directive with typed data island of results

**REVIEW** - Start comprehensive code and design review session, analyzing architecture, patterns, and improvements

**BUILD** - Plan and negotiate sister app creation to automate repetitive LLM workflows with fast shell execution

**DOCUMENT** - Generate comprehensive documentation from code and comments with intelligent structure

**COHERE** - Align code and comments for consistency, ensuring documentation matches implementation

**QUERY** - Execute SSQQLL query and replace directive with typed data island of results

*Note: All programming commands (prettify, canonicalize, compile, optimize, discuss, review, build, document, cohere) are part of the [CCLLII interface](#ccllii-chat-command-large-language-interactive-interface) and can be used directly in prompts or with COMMAND prefix.*

#### **Magic Invisible Keys**

**TODO** - Development notes that LLM tracks for queries like "show all TODOs" but are ignored in exports

**COMMENT** - Invisible documentation that LLM understands for context but stays invisible to SSQQLL and exports

#### **Command Processing Rules**

1. **Command Detection**: Any "COMMAND", "QUESTION", or "TRY" key triggers processing
2. **Command Execution**: Perform specified transformation/analysis
3. **Token Stripping**: Remove command/token from ID or strip command key entirely
4. **State Update**: Replace original data with transformed result
5. **Audit Trail**: Optionally preserve command history

**🚨 CRITICAL SAFETY RULE: DO NOT EXECUTE OR STRIP COMMANDS OR TOKENS IN QUOTES!**

Quoted tokens are for documentation and examples only. The LLM must never process quoted commands as executable instructions. This prevents accidental execution of documentation examples.

**Processing Details:**
- **Space-separated tokens**: `id: "original_id DELETE"` → command applied, token stripped → `id: "original_id"`
- **Command key**: `COMMAND: "ALPHABETIZE"` → command applied, key removed entirely
- **Case flexibility**: Commands can be any case - `"delete"`, `"DELETE"`, `"Delete"` all work
- **Fuzzy matching**: `"ABC"`, `"ALPHABETIZE"`, `"ALPHABETISE"`, `"ALPHABETIZE"` all trigger alphabetization
- **Key preservation**: Only the "COMMAND" key itself must be uppercase, command values are flexible

#### **Batch Command Processing**

Multiple commands can be inserted throughout a document and processed all together in intelligent dependency order.

**Intelligent Command Ordering:**

```yaml
# Commands execute in logical dependency order:
command_execution_order:
  1. "VALIDATE"      # Check data integrity first
  2. "MERGE"         # Combine data before other operations
  3. "GENERATE"      # Create new objects
  4. "ALPHABETIZE"   # Organize and clean up
  5. "CLONE"         # Create instances
  6. "DEPRECATED"    # Mark for phase-out
  7. "ARCHIVE"       # Preserve before removal (happens after editing)
  8. "DELETE"        # Final removal (last step)
  
# Interactive commands process separately:
interactive_commands:
  - "QUESTION"       # Analyze and answer
  - "TRY"           # Preview operations
```

**Batch Processing Benefits:**
- **Document-wide operations**: Apply transformations across entire document at once
- **Dependency resolution**: Commands execute in logical order automatically
- **Atomic operations**: All commands succeed or fail together
- **Edit-then-archive**: Editing commands complete before archival
- **Cleanup workflows**: Perfect for document maintenance and reorganization

#### **COMMAND as Prefix Token and Object Key**

**"COMMAND" can be used in multiple ways for maximum flexibility:**

**1. Prefix Token for Natural Language Commands:**
Place "COMMAND" at the start of any line to issue natural language instructions that apply to surrounding content.

**2. Key in YAML/JSON Objects:**
Use "COMMAND" as a key within any YAML or JSON structure to focus the command context on just that specific object or sub-structure.

**3. Reference Tokens with THIS/THERE:**
Use "THIS" and "THERE" tokens to create precise references for moving, copying, or transforming content between locations. Both tokens support optional labels for multi-target operations.

**Basic Reference Patterns:**
- `"COMMAND": "put THIS THERE"` - Move content from THIS location to THERE location
- `"COMMAND": "copy THIS to THERE"` - Copy content between marked locations  
- `"COMMAND": "merge THIS with THERE"` - Combine content from two marked locations
- `"COMMAND": "swap THIS THERE"` - Exchange content between two locations

**Labeled Reference System:**
Use labeled tokens to create multiple named references and perform complex multi-target operations:

- `"THIS": "cat"` and `"THIS": "dog"` - Mark multiple source locations with labels
- `"THERE": "box"` and `"THERE": "shelf"` - Mark multiple destination locations with labels  
- `"COMMAND": "swap cat dog"` - Swap content between labeled locations
- `"COMMAND": "combine foo and bar to box"` - Merge multiple sources to one destination
- `"COMMAND": "move cat to shelf and dog to box"` - Multiple operations in one command

**Multi-Target Processing:**
The pattern works by:
1. **Mark locations** with "THIS" and "THERE" tokens (with optional labels)
2. **Issue commands** referring to any combination of labeled references
3. **Process in lexical order** - commands execute in document order
4. **Strip all markers** - remove all "THIS", "THERE", and "COMMAND" tokens after processing

**Advanced Command Examples:**
- `"COMMAND": "duplicate cat to box and shelf"` - Copy one source to multiple destinations
- `"COMMAND": "merge all THIS locations into summary"` - Combine all marked sources
- `"COMMAND": "sort cat dog elephant alphabetically"` - Reorder multiple labeled items
- `"COMMAND": "validate foo bar and baz together"` - Multi-object validation

**Object-Focused Commands:**
You can place a "COMMAND" key within any YAML or JSON object to focus the command scope on just that object or sub-structure. For example, a stats sub-object could have its own "COMMAND" to validate just those stats, while an inventory array could have its own "COMMAND" to alphabetize just that list.

**Collection Reference Tokens:**
For working with groups and collections of items:

- `"THESE fruits"` - Mark a collection of source items with label
- `"THOSE vegetables"` - Mark another collection of source items  
- `"THAT pantry"` - Mark a destination collection with label

**Collection Command Examples:**
- `"COMMAND move THESE to THAT"` - Move entire collection to destination
- `"COMMAND merge THESE and THOSE into THAT"` - Combine multiple collections
- `"COMMAND sort THESE alphabetically"` - Reorder items within a collection
- `"COMMAND duplicate THESE to pantry and fridge"` - Copy collection to multiple destinations
- `"COMMAND split THESE into ripe and unripe"` - Divide collection by criteria

**Virtual Selection and Manipulation System:**

**COPY/MOVE Commands:**
- **COPY** - Copy content from source to destination with optional translation/formatting
- **MOVE** - Move content from source to destination with optional translation/formatting

**Structure-Aware vs Text-Based Operations:**

**Default: Structure-Aware Editing**
Commands interpret content as structured data first:
- **JSON/YAML objects** - Move entire objects, merge properties, restructure hierarchies
- **Markdown headers** - Move sections with all subsections, reorder outline structure
- **Lists and outlines** - Move list items, reorder bullets, restructure hierarchies
- **Tables** - Move rows/columns, merge tables, restructure data
- **Code blocks** - Move entire blocks, preserve syntax and formatting

**TEXT Token for Plain Text Operations**
Add "TEXT" token to force plain text editing instead of structural operations:
- `MOVE TEXT HERE content HERE to THERE` - Treats content as raw text
- `COPY TEXT THIS paragraph THIS to THAT` - Ignores markdown structure
- `TEXT MOVE source to destination` - Plain text operation throughout

**Position Markers:**
- **"HERE"** - Mark single position for insertion/reference
- **"THERE"** - Mark another single position for insertion/reference

**Paired Selection Tokens:**
Point selection tokens can work in pairs like quotes to select content:
...
raw_config = COPY: config.yaml LITERAL
