# 🎯 PutThatThere: Universal Content Management Command Overlay
...
### 🎒 Spatial Container Operations (Ted Nelson Approved!)
...
#### **📐 measure** - Text analysis and metrics
```
...
### 📋 Document-Embedded Interface Elements
...
→ Creates: [REPLACE HERE character_name] at target location
→ Creates: [BEGIN MOVE character_name]Reginald[END MOVE character_name] at source
→ Executes replacement

"Put this business data in the revenue field"
→ THAT: [BEGIN MOVE revenue_data]selected_content[END MOVE revenue_data]
→ THERE: json_structure.revenue[REPLACE HERE revenue_data]
→ PUT THAT THERE

"Drop this text in the middle of that description"
→ Creates appropriate [REPLACE HERE] tag in middle of target text
→ Creates matching [BEGIN MOVE] tag around source content
→ Executes surgical replacement
```
...
→ Auto-generates: [REPLACE HERE auto_tag_1] and [BEGIN MOVE auto_tag_1]

"Put the character data in multiple places"
→ Auto-generates: [REPLACE HERE char_data_1], [REPLACE HERE char_data_2]
→ Single source: [BEGIN MOVE char_data_1] (automatically duplicated)

# User can specify custom tag names
"Replace the revenue field with this data, call it Q4_revenue"
→ Creates: [REPLACE HERE Q4_revenue] and [BEGIN MOVE Q4_revenue]
```
...
[REPLACE HERE revenue_data] ← Must have matching source
[BEGIN MOVE revenue_data]...[END MOVE revenue_data] ← Must have matching destination

# Orphaned tags generate warnings
[REPLACE HERE missing_tag] ← Warning: No matching source found
[BEGIN MOVE unused_tag]...[END MOVE unused_tag] ← Warning: No matching destination

# Auto-completion for tag names
Type: [REPLACE HERE rev → Auto-suggests: revenue, revenue_data, revenue_model
```

**Type Safety in Replacements:**
```
# System validates type compatibility
[REPLACE HERE number_field] ← Expects numeric value
[BEGIN MOVE number_field]"text string"[END MOVE number_field] ← Warning: Type mismatch

# Auto-conversion when possible
[REPLACE HERE number_field] ← Expects number
[BEGIN MOVE number_field]"123.45"[END MOVE number_field] ← Auto-converts to 123.45

# Schema validation for complex structures
[REPLACE HERE character_object] ← Expects character schema
[BEGIN MOVE character_object]{"invalid": "structure"}[END MOVE character_object] ← Schema validation error
```
